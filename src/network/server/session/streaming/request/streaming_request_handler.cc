...
void StreamingRequestHandler::handle_auth()
{
    if (has_write_auth) {
        const auto msg = "Auth Exception: Already authorized";
        logger(Category::Error) << msg;
        response_writer->write_error(msg);
        response_writer->flush();
        return;
    ...
    request_reader->check_datatype(Protocol::DataType::STRING);
    const auto user = request_reader->read_string();
    request_reader->check_datatype(Protocol::DataType::STRING);
    const auto pass = request_reader->read_string();

    const auto&& [auth_token, valid_until] = session.server.create_auth_token(user, pass);

    if (auth_token.empty()) {
        const auto msg = "Auth Exception: Invalid user and/or password";
        logger(Category::Error) << msg;
        response_writer->write_error(msg);
        response_writer->flush();
        return;
    ...
    has_write_auth = true;
    response_writer->write_auth_success();
    response_writer->flush();
}
